  ...
  .object({
    ...
    disableThinking: z.boolean().optional().openapi({
      description:
        ...
        "Kimi K2.6 1,173 → 452, same answers). Pass `false` to keep reasoning ON for a structured " +
        "request; pass `true` to turn it off for a free-text one. Omitted = the service default " +
        "(bounded thinking on the native providers, byte-identical to a normal call; reasoning off " +
        "for structured vendor calls, provider-normal for text ones). A vendor model that refuses " +
        "the option fails loud with a 400 in its own words rather than silently reasoning anyway.",
      example: true,
    }),
    thinkingLevel: z.enum(["minimal", "low", "medium", "high"]).optional().openapi({
      description:
...
export const GenerateImageResponseSchema = z
  .object({
    url: z.string().url().openapi({
      description: "Cloudflare-hosted URL for the generated image. Store and serve this URL; generated bytes are not returned.",
      example: "https://cdn.distribute.you/images/audience-avatar.png",
    }),
    mimeType: z.string().openapi({
      description: "MIME type for the stored generated image.",
      example: "image/png",
    }),
    model: z.string().openapi({
...
Chat-service owns provider key resolution, Gemini request shaping, run creation, cost provisioning, billing authorization, provider execution, Cloudflare storage, and cost reconciliation. Callers pass a text prompt plus optional size and receive a hosted image URL plus MIME/model metadata suitable for storage.`,
  request: {
    headers: z.object({
      "x-api-key": z.string().openapi({
        description: "Service-to-service API key",
  ...
  responses: {
    200: {
      description: "Generated hosted image",
      content: {
...
Callers pass a text prompt plus optional size and receive a hosted image URL plus MIME/model metadata suitable for storage.`,
  request: {
    headers: z.object({
      "x-api-key": z.string().openapi({
        description: "Service-to-service API key",
