...
> **Note — `/complete` floors Gemini 3 thinking by default.** Because `/complete` is for extraction / scoring / structured output (never deep reasoning) and Gemini 3.5 Flash flakes on large structured outputs when thinking is enabled ([googleapis/js-genai#1619](https://github.com/googleapis/js-genai/issues/1619) — thinking + the schema-constrained decode exhaust the 64k budget → `MAX_TOKENS` / malformed JSON), every Gemini 3 model is automatically dropped to its thinking floor on this endpoint (`thinkingLevel: "minimal"` for Flash, `"low"` for Pro) regardless of `disableThinking`. Gemini 2.5 keeps a small thinking budget unless `disableThinking` zeroes it. `/chat` is unaffected.

- `disableThinking` (optional, default `false`) — minimize the model's internal reasoning ("thinking") so the whole output budget goes to the answer. Use for extraction / structured-JSON / scoring tasks that don't need chain-of-thought. **Provider-floored, NOT a guaranteed full-off** (same pattern as a per-provider cap). Note: on `/complete`, Gemini 3 is *already* floored (see the note above), so this flag only changes behavior for Gemini 2.5 (→ fully off):
