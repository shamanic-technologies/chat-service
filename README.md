...
- `json` — parsed JSON object (present when `responseFormat: "json"` or when `responseSchema` is set). Populated by parsing the model output — no jsonrepair, no LLM-assisted repair. The happy path is a bare JSON value (`JSON.parse(content.trim())`). Provider-native enforcement (`output_config.format` for Anthropic, `responseMimeType` / `responseSchema` for Gemini) is *not* a hard guarantee: Gemini 3 thinking models intermittently append trailing prose (or wrap the value in a markdown fence) despite JSON-mode metadata, so on a strict-parse failure the service recovers the **first complete JSON value** — it strips a leading ```` ``` ````/```` ```json ```` fence, then runs a string-aware balanced-delimiter scan from the leading `{`/`[` to its matching close and parses that substring, ignoring anything after it. Recovery is narrow: it applies only to a fence OR trailing content after a value that starts the output. Leading non-fence prose, empty output, and truncated/unbalanced JSON still fail loud with a classified `detail` and the endpoint returns **502**.
...
When `jsonMode` is set, the service parses the model output to populate `response.json`. It first tries a strict `JSON.parse(content.trim())`. Because Gemini 3 thinking models can leak trailing prose (or a markdown fence) despite JSON-mode metadata, a strict-parse failure triggers **first-value recovery**: strip a leading ```` ``` ````/```` ```json ```` fence, then a string-aware balanced-delimiter scan from the leading `{`/`[` to its matching close, parse that substring, and ignore anything after it. This is *not* jsonrepair — no malformed JSON is rewritten. Recovery covers only a fence or trailing content after a value that starts the output; leading non-fence prose, empty output, and truncated/unbalanced JSON still fail loud (502) with a classified `detail`.
