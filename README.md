...
| `audience-editor` | `list_audiences`, `suggest_audiences`, `set_audience_status`, `rename_audience`, `refresh_audience_count`, `request_user_input` | Create + curate a brand's customer audiences via NL (human-service, via the api-service gateway `/v1/orgs/audiences/*`). Creation is suggest→activate: `suggest_audiences` persists candidates at status `suggested` (invisible to the customer until activated, so never announced as created); `set_audience_status … active` makes one live. Candidates carry `context.offerId` when the chat request has one. Filters are immutable; archive (never delete) and rename only. |
...
- `provider` (required) — LLM provider: `"anthropic"`, `"google"`, `"deepseek"`, `"zai"`, `"moonshot"`, or `"openai"` (see [Direct vendor models](#direct-vendor-models))
- `model` (required) — version-free model alias. The service resolves the current versioned model internally. Valid combinations:
  ...
  The four direct-vendor providers are **text only**: `imageUrl` and `webSearch` are rejected with 400 on every one of their models. Each needs its OWN key in key-service, stored under its provider slug.
...
- `temperature` (optional) — sampling temperature, 0–2 (default: model default). **Rejected with a 400 by the reasoning-first models that removed the sampling parameters** — today `anthropic`/`fable` and `openai`/`gpt-pro`. Anthropic dropped them on its always-thinking models and OpenAI dropped them on its reasoning models, so chat-service refuses the pair at the route, before any cost is held, and the error names the field to drop (plus, on Anthropic, the sibling aliases that still accept it — on a direct vendor it is a per-vendor fact, so there is no sibling to suggest). The parameter is never silently dropped: answering 200 from a model sampling differently from what the caller asked for is the same quiet wrongness as serving a fallback model.
...
- `disableThinking` (optional, **tri-state**) — minimize the model's internal reasoning ("thinking") so the whole output budget goes to the answer. Use for extraction / structured-JSON / scoring tasks that don't need chain-of-thought. **Provider-floored, NOT a guaranteed full-off** (same pattern as a per-provider cap):
  ...
  - **Direct vendors** (`deepseek`, `zai`, `moonshot`) → reasoning fully **OFF**; **`openai`** → floored at `reasoning_effort: "low"`, the lowest GPT-6 Astra accepts (`none` and `minimal` are 400s on that model, so there is no full-off — "minimize", exactly like Gemini 3). It is **already off by default for any request that asks for structured output** (`responseSchema`, or `responseFormat: "json"`) — those callers parse the object and never see the reasoning, which is billed as output tokens. Pass `false` to keep reasoning on for a structured request; pass `true` to turn it off for a free-text one. See [Reasoning is off by default for structured output](#reasoning-is-off-by-default-for-structured-output).
...
`provider: "anthropic"`, `model: "fable"` → `claude-fable-5-1`, cost prefix `anthropic-fable-5.1`. Added 2026-09-09; the tier **above** Opus and priced there ($10 / $50 per 1M, against Opus 4.6's rates), 1M context. It runs on the existing native Anthropic client — no new transport — but three of its API behaviours differ from `haiku` / `sonnet` / `opus`, and all three are 400s rather than degradations:

| | Behaviour on `fable` | What chat-service does |
|---|---|---|
| Sampling | `temperature`, `top_p`, `top_k` all removed — Anthropic answers `400 invalid_request_error: "`temperature` is deprecated for this model."` (reproduced live 2026-09-09; the identical request without it returns 200). OpenAI did the same on its reasoning models, so `openai`/`gpt-pro` is refused identically — see [Direct vendor models](#direct-vendor-models) | Refused at the route **before any cost is held**, with a message naming the field to drop and the aliases that still accept it. `anthropicRejectsSampling` is per-**model** data, never inferred from the family — the same shape as `GEMINI_3_THINKING_FLOOR`. |
...
A third provider path alongside the two native clients. Where `anthropic.ts` and `gemini.ts` each speak a vendor-specific dialect and carry that vendor's accumulated quirks, `src/lib/openai-compatible.ts` speaks one generic dialect — OpenAI Chat Completions — against vendors that all serve it. **One adapter, four vendors, seven models.**

The vendors differ in exactly eight things, and all eight are *data* in the `VENDORS` registry rather than code: the base URL, the key-service provider slug, where the usage payload reports cached prompt tokens, **which price dimensions that vendor's catalog actually carries** (`pricing`), **which `response_format` it implements** (`structuredOutput`), **what it calls the output-token cap** (`maxOutputTokensField`), **whether it accepts the sampling parameters** (`sampling`), and **whether its models' reasoning can be turned off, and what that field is called** (`reasoning`). So adding an eighth model is a `MODEL_MAP` entry, and adding a fifth vendor is a `VENDORS` entry. Neither is a new client.

OpenAI joined on 2026-09-09 and is the interesting case, because it is the vendor whose dialect the other three imitate — this adapter reaches `api.openai.com`'s own `/chat/completions`, so "OpenAI-compatible" is not an approximation there. It still needed three of the eight per-vendor facts to differ from all three incumbents — `max_completion_tokens` instead of `max_tokens`, a reasoning floor rather than an off switch, and no sampling at all — which is the argument for the registry rather than against it.
...
**The schedule is a deliberate local copy.** The windows and the day scope live in this repo, on `VENDORS.deepseek.pricing.regime` — chat-service does not read the catalog at runtime to decide which regime a call falls in. That keeps a cost name buildable without a round trip, and it means the vendor's rule is written down in two repos: here, and in costs-service beside the price points, in that service's own grammar. Neither copy is derived from the other by transforming a string. A vendor that moves its hours or its days is a change in **both** — read the vendor's pricing page, then update both.
...
chat-service does **not** send OpenAI's `strict: true`. Strict mode additionally requires `additionalProperties: false` on every object node and every property listed in `required` — that is the *Anthropic* dialect (`prepareAnthropicSchema`), not the shape a caller's schema arrives in here, and the two normalizers are deliberately mirror images that must not be merged. Non-strict `json_schema` is what the vendor serves without rewriting the caller's schema; `parseModelJsonOutput` still fails loud (502) on output it cannot read.
...
**OpenAI floors rather than disables.** GPT-6 Astra accepts `reasoning_effort` and refuses `none`, `minimal` **and `max`** — `400 unsupported_value: "… Supported values are: 'low', 'medium', 'high', and 'xhigh'."` That is **stricter than the model page**, which also lists `max`; the value here comes from the live API, which is the whole reason it is probed rather than read. So `low` is the floor, exactly as on the GLM-5.3 family, and `disableThinking` keeps the meaning it has everywhere else here: **minimize, not zero**.

It matters more on this vendor than on any incumbent — at **$50 per 1M output tokens** Astra has the most expensive output rate of any model this service reaches, and reasoning is billed at that rate for a monologue the caller never sees. Measured on one prompt, 2026-09-09:

| Request | Completion tokens | of which reasoning |
|---|---:|---:|
| baseline (no effort sent) | 73 | **52** |
| `reasoning_effort: "low"` | 16 | **0** |

Worth recording how close this came to staying guesswork: the first probe run that day hit an empty balance, and OpenAI's quota gate sits **in front of** parameter validation — a request carrying a deliberately invalid `reasoning_effort: "none"` came back `429 credit_balance_exhausted`, not `400`. An empty balance therefore makes every capability unprobeable rather than merely making completions fail.
...
**OpenAI publishes no in-flight limit at all** — what it publishes is a request and token *rate* per account tier, which is a different quantity. `publishedConcurrency("openai", ...)` therefore answers `null` by construction, and that null must not be filled in from the RPM column; the rate is recorded verbatim under a `per-account-rate` scope so the ceiling we are actually bounded by sits next to the price. Unlike the GLM-5.3 case this is not a throughput risk at our volume: even Tier 1's 500 requests per minute is orders of magnitude above what a cold-email workflow asks for, and it is a rate ceiling rather than a single slot three campaigns have to queue behind. Which tier this account is on is a console fact the API does not report, so no tier is asserted in the registry.
...
1. **One key per vendor** in key-service, stored under that vendor's slug: `deepseek`, `zai`, `moonshot`, `openai`. Org-scoped calls read `GET /keys/{provider}/decrypt`; `/internal/platform-complete` reads `GET /keys/platform/{provider}/decrypt`. A missing key returns **502** naming the vendor and the slug it must live under — not a generic failure — so "which of the three is unconfigured" is answered by the error itself.
2. The catalog rows for the alias live in production costs-service. As of 2026-08-16 every reachable model is priced: the twelve regime-and-cache DeepSeek rows, and three rows per model (`-tokens-{input,cached-input,output}`) for both Z.ai and Moonshot. OpenAI's three `openai-gpt-6-astra-tokens-*` rows and the `openai` `pay-as-you-go` plan row went live on 2026-09-09 (costs-service v0.55.0), alongside `anthropic-fable-5.1-tokens-*` on the native path. An alias added ahead of its rows still returns **502** naming the three rows someone must seed (`missingCostNames` in the body), before any key fetch, run or vendor call — a wrong price is silent, a missing one is not. Seed them in costs-service `src/db/seed.ts` (`SEED_PROVIDERS_COSTS`) and deploy to production to lift it.

### Out-of-credit alerting

Each vendor holds its own prepaid balance, and most have no auto-reload. When a balance runs dry the vendor refuses every call and the whole model tier stops answering, so **chat-service raises the fleet's staff alert, naming the vendor**.
...
| OpenAI | HTTP 429 with code **`credit_balance_exhausted`** / type **`insufficient_quota`**, "You have no credits remaining. Add credits to continue using the API" | HTTP 429, code `rate_limit_exceeded` |

OpenAI's row is the one captured verbatim from a live refusal (2026-09-09, `gpt-6-astra` on an empty account) rather than from docs, and it is pinned as a fixture — the shared prose fallback below does **not** fire on its wording, so the structural signal is all there is.

A shared prose fallback (`insufficient balance` / `run out of balance` / `no resource package`) catches a vendor that rewords its body without changing its code — OpenAI's wording matches none of it, which is why its predicate reads `code`/`type` and both are matched rather than either alone. Adding a fifth vendor means adding its `isOutOfCreditRefusal` predicate to its `VENDORS` entry — the test table in `tests/unit/vendor-credit-alert.test.ts` fails until it is there.
...
**Audience-editor tools** (operate on the brand from `context.brandId`, and — for `suggest_audiences` — the offer from the optional `context.offerId`; via human-service through the api-service gateway `/v1/orgs/audiences/*`, org-scoped by the forwarded `x-org-id`). `context.offerId` is never derived from anything else: absent ⟹ audiences are created brand-wide, exactly as before.
...
| `suggest_audiences` | Proposes candidate audiences from a natural-language `nlPrompt`. Each candidate is persisted at status `suggested` — **invisible to the customer** (no product surface lists a `suggested` audience), so the model must never report it as a created audience; activation is a separate step. Scoped to `context.offerId` when the request carries one (the dashboard sends it only from an offer-scoped page), so the candidates appear on that offer's audiences page once activated; omitted ⟹ brand-wide, unchanged. `POST /v1/orgs/audiences/suggest` |
