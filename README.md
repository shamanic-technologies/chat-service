...
- `temperature` (optional) — sampling temperature, 0–2 (default: model default). **Rejected with a 400 by models that removed the sampling parameters** — today that is `anthropic`/`fable`. Anthropic dropped `temperature`, `top_p` and `top_k` on its always-thinking models, so chat-service refuses the pair at the route, before any cost is held, and the error names the field to drop and the aliases that still accept it. The parameter is never silently dropped: answering 200 from a model sampling differently from what the caller asked for is the same quiet wrongness as serving a fallback model.
...
|---|---|---|
| Sampling | `temperature`, `top_p`, `top_k` all removed — Anthropic answers `400 invalid_request_error: "`temperature` is deprecated for this model."` (reproduced live 2026-09-09; the identical request without it returns 200) | Refused at the route **before any cost is held**, with a message naming the field to drop and the aliases that still accept it. `anthropicRejectsSampling` is per-**model** data, never inferred from the family — the same shape as `GEMINI_3_THINKING_FLOOR`. |
...
The vendors differ in exactly seven things, and all seven are *data* in the `VENDORS` registry rather than code: the base URL, the key-service provider slug, where the usage payload reports cached prompt tokens, **which price dimensions that vendor's catalog actually carries** (`pricing`), **which `response_format` it implements** (`structuredOutput`), **what it calls the output-token cap** (`maxOutputTokensField`), and **whether its models' reasoning can be turned off, and what that field is called** (`reasoning`). So adding an eighth model is a `MODEL_MAP` entry, and adding a fifth vendor is a `VENDORS` entry. Neither is a new client.

OpenAI joined on 2026-09-09 and is the interesting case, because it is the vendor whose dialect the other three imitate — this adapter reaches `api.openai.com`'s own `/chat/completions`, so "OpenAI-compatible" is not an approximation there. It still needed two of the seven per-vendor facts to differ from all three incumbents (`max_completion_tokens` instead of `max_tokens`, and a reasoning floor rather than an off switch), which is the argument for the registry rather than against it.
...
| `openai` | `{ type: "json_schema", ... }` | Structured Outputs is a first-class OpenAI feature and the [model page](https://developers.openai.com/api/docs/models/gpt-6-astra) lists it as supported. Not verified against the live API — the account had no credit on 2026-09-09 |
...
**OpenAI floors rather than disables.** GPT-6 Astra accepts `reasoning_effort: low | medium | high | xhigh | max`, and the reasoning guide is explicit that it "does not support `none` reasoning effort. Setting `reasoning.effort` to `none` returns HTTP 400" — `minimal` is in OpenAI's general set but absent from this model's. So `low` is the floor, exactly as it is on the GLM-5.3 family, and `disableThinking` keeps the meaning it has everywhere else here: **minimize, not zero**. It matters more on this vendor than on any incumbent — at **$50 per 1M output tokens**, Astra has the most expensive output rate of any model this service reaches, and reasoning is billed at that rate for a monologue the caller never sees.

This value is taken from the vendor's docs and is **not measured**: the account had no credit on 2026-09-09, and a probe carrying a deliberately invalid `reasoning_effort: "none"` came back `429 credit_balance_exhausted` rather than a `400`, proving the quota gate sits *in front of* parameter validation. No capability can be probed from that account until it is topped up — re-probe then.
...
**OpenAI publishes no in-flight limit at all** — what it publishes is a request and token *rate* per account tier, which is a different quantity. `publishedConcurrency("openai", ...)` therefore answers `null` by construction, and that null must not be filled in from the RPM column; the rate is recorded verbatim under a `per-account-rate` scope so the ceiling we are actually bounded by sits next to the price. Unlike the GLM-5.3 case this is not a throughput risk at our volume: Tier 1's 500 requests per minute is orders of magnitude above what a cold-email workflow asks for, and it is a rate ceiling rather than a single slot three campaigns have to queue behind.
