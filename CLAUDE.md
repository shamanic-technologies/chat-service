...
## Vercel AI Gateway (`provider: "vercel"`) — a THIRD path, never a replacement

`src/lib/gateway.ts` is an OpenAI-compatible client against `https://ai-gateway.vercel.sh/v1/chat/completions`, wired into `/complete` + `/internal/platform-complete` ONLY. It exists so reaching a new model is a `MODEL_MAP` entry instead of a fourth hand-written native client. **Do NOT remove or degrade the Anthropic/Google native paths to "consolidate" onto it** — they carry vendor features the gateway path deliberately does not serve (thought signatures, the two mirror-image schema normalizers, per-generation thinking config, grounding, vision).

**Model breadth is NOT catalog breadth.** The gateway serves ~330 models; chat-service reaches only aliases declared in `MODEL_MAP.vercel`, and `resolveModel` throws on anything else. Every alias costs exactly TWO costs-service catalog rows (`<costPrefix>-tokens-input`/`-tokens-output`) that must be live in PRODUCTION before the alias ships, or runs-service 422s and the call fails loud. Never "enumerate the gateway catalog" — add one alias + its two rows, deliberately. Cost names are seeded in **costs-service `src/db/seed.ts`** (`SEED_PROVIDERS_COSTS`, upserted at that service's boot); `PUT /v1/providers-costs/{name}` only adds a price point to a name that ALREADY exists, so a new name needs a costs-service PR, not an API call.

**Pricing rule — PEAK rate, and cache reads are billed at full price.** DeepSeek splits peak (01:00–04:00, 06:00–10:00 UTC) vs off-peak at half, and the gateway routes across ~10 providers of the same model whose rates differ ~3×. The catalog carries the PEAK list price, above every provider's current rate, so cheap hours and cheap routing are margin, never shortfall (same rule as pricing Gemini post-promotion). `cachedInputTokens` is captured for OBSERVABILITY ONLY and must NOT be discounted: AI Gateway currently bills implicit-cache tokens at full input price for OpenAI/DeepSeek-class providers ([vercel/ai#13907](https://github.com/vercel/ai/issues/13907), open), and no provider has a `-tokens-cached-input` catalog name today (Anthropic cache reads are already billed as full input). **Do NOT add a cache-discounted cost path until BOTH change** — and never estimate a cache split that the response didn't report.

**Never add routing knobs to the request body.** No `models` fallback array, no `sort`, no provider `order`/`only`. The gateway would then be free to serve a DIFFERENT model than the alias we priced, and the cost row would name a model that didn't run. `assertModelMatches` enforces the other half — it throws when the response's `model` is not the requested one. **Do NOT "improve reliability" by adding a fallback model list**; that trades a loud failure for a silently mis-declared cost.
...
**Text-only by design.** `webSearch` / `imageUrl` with `provider:"vercel"` return 400 rather than silently answering ungrounded or blind. `/chat` is NOT wired to this provider and its config schemas (`AppConfigRequest`/`PlatformConfigRequest`) deliberately do NOT accept `"vercel"` — agentic tool-calling reliability on these models is unproven against our tool set and must be measured in a bake-off before any default moves.
