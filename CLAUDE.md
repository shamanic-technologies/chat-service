...
`completeWithGemini` MUST set `generationConfig.thinkingConfig = buildThinkingConfig(model)` (`src/lib/gemini.ts`). Gemini 3 thinking tokens count against `maxOutputTokens`, and a Gemini 3 model with NO thinkingConfig defaults to HIGH thinking — it spends the entire 64k output budget reasoning internally and emits only 7-9 visible tokens before `finishReason: MAX_TOKENS`, so EVERY jsonMode `/complete` 502s (incident 2026-06-24, #316: brand-service ICP-suggest / human-service audiences-suggest / extract-fields all down → onboarding generation dead). `buildThinkingConfig` is generation-specific: **Gemini 3.x (`gemini-3*`, incl `gemini-3.5-flash`) → `{ thinkingLevel: "low" }`; Gemini 2.5 → `{ thinkingBudget: 8192 }`.** It is the SAME helper the `/chat` path uses (`gemini-chat.ts`, commit 1991af2) — defined once in `gemini.ts`, imported by `gemini-chat.ts` (no circular import: gemini-chat already depends on gemini). **Never remove `thinkingConfig` from the `/complete` body** (that omission is the #316 bug), and **never "disable" Gemini-3 thinking with `thinkingBudget: 0`** (PR #133's value) — on Gemini 3 `thinkingBudget` is only accepted for backwards-compat and produces the same degenerate empty/truncated output; the disable knob for Gemini 3 is `thinkingLevel`, not `thinkingBudget`. This is a SEPARATE lever from the `maxOutputTokens` cap above — raising the cap alone never fixed it (thinking just consumed the larger budget too, exactly why #131's 16k→64k bump didn't help).

**`/complete` FLOORS Gemini 3 thinking by default — `thinkingLevel:"low"` was NOT low enough (incident 2026-06-24, #316 follow-up).** The first #316 fix set Gemini 3 to `thinkingLevel:"low"` (mirroring `/chat`). That FAILED in prod — `gemini-3.5-flash` on a 17-field brand-extraction (`tokensInput≈1665`) still hit `MAX_TOKENS` at ~8 output tokens, DETERMINISTICALLY, all 3 brand-service retries. Root cause is a **known Gemini 3.5 Flash model bug** ([googleapis/js-genai#1619](https://github.com/googleapis/js-genai/issues/1619)): on LARGE structured outputs (big `responseSchema` / function-call args) the model flakes across three modes — `MAX_TOKENS` exhaustion, `MALFORMED` (the sibling "trailing non-JSON content" 502 on the `pro` path), or slow success — because thinking + the schema-constrained decode exhaust the 64k budget before any JSON is emitted, and `"low"` thinking for `gemini-3.5-flash` is still large. **Fix:** `completeWithGemini` FLOORS Gemini 3 thinking on `/complete` — `minimizeThinking = disableThinking || model.startsWith("gemini-3")` → `buildThinkingConfig(model, minimizeThinking)` → `"minimal"` (Flash) / `"low"` (Pro floor). `/complete` is extraction/scoring/structured-output and never needs CoT, so flooring is safe and frees the whole budget for the decode. **Never raise the `/complete` Gemini-3 baseline back to `"low"`** — that is the deterministic-failure value. `/chat` is unaffected (keeps `"low"` for tool-calling; does not call `completeWithGemini`). NOTE: flooring is a MITIGATION — `#1619` is still flaky on very large outputs; the durable fix is to split oversized single-call extractions into smaller schemas (producer-side, e.g. brand-service's 17-field call) or move that extraction to `gemini-2.5-flash` (no 3.x structured regression, `thinkingBudget:0` real-off).
