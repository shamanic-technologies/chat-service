...
**Caller knob — `disableThinking` (POST /complete + /internal/platform-complete) is PROVIDER-FLOORED, never a guaranteed zero.** Callers can pass `disableThinking: true` to minimize thinking (extraction / JSON / scoring tasks). `buildThinkingConfig(model, disableThinking)` resolves it: **Gemini 2.5 → `thinkingBudget: 0` (fully off); Gemini 3 → the lowest level the gen allows — `thinkingLevel: "minimal"` (Flash/flash-lite, incl. the `flash-pro` = `gemini-3.5-flash` default), `"low"` (Pro); Anthropic → no-op** (the `/complete` Anthropic path never enables thinking — the `adaptive`/`clear_thinking` config is `/chat`-only). **Gemini 3 has NO full-off** (verified: [thinking docs](https://ai.google.dev/gemini-api/docs/thinking) — Flash floors at `minimal`, Pro at `low`), so document it as "minimize", exactly like the `maxSearches` cap (hard knob on one provider, best-effort on the other) — do NOT promise a literal zero on Gemini 3, and do NOT try `thinkingBudget: 0` on Gemini 3 (a backwards-compat no-op that re-degenerates the output). `/chat` does NOT expose the `disableThinking` knob — its agentic loop needs thinking for tool-calling.
