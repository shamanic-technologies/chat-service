...
describe("POST /complete — direct vendor routing", () => {
  ...
  const CASES = [
    { provider: "deepseek", model: "deepseek-flash", host: DEEPSEEK_URL, wire: "deepseek-flash", prefix: "deepseek-flash-peak" },
    { provider: "deepseek", model: "deepseek-pro", host: DEEPSEEK_URL, wire: "deepseek-flash", prefix: "deepseek-flash-peak" },
  ...
  it("bills DeepSeek cache hits under the cached-input name, not the miss rate", async () => {
    ...
    expect(quantityOf(actual, "deepseek-flash-peak-tokens-input")).toBe(40);
    expect(quantityOf(actual, "deepseek-flash-peak-tokens-cached-input")).toBe(960);
    expect(quantityOf(actual, "deepseek-flash-peak-tokens-output")).toBe(20);
    // The split is exhaustive: nothing is billed twice, nothing is dropped.
    expect(
      quantityOf(actual, "deepseek-flash-peak-tokens-input")! +
        quantityOf(actual, "deepseek-flash-peak-tokens-cached-input")!,
    ).toBe(1000);
    // The superseded flat names are frozen in the catalog and never declared.
    expect(actual.some((i) => i.costName === "deepseek-flash-tokens-input")).toBe(false);
    expect(actual.some((i) => i.costName === "deepseek-flash-tokens-cached-input")).toBe(false);
    expect(actual.some((i) => i.costName === "deepseek-flash-tokens-output")).toBe(false);
  ...
  it("declares DeepSeek against the off-peak names one minute after a peak window closes", async () => {
    ...
    expect(cap.postedItems[0].map((i) => i.costName).sort()).toEqual([
      "deepseek-flash-off-peak-tokens-input",
      "deepseek-flash-off-peak-tokens-output",
    ]);
    const actual = actualItems(cap.postedItems);
    expect(quantityOf(actual, "deepseek-flash-off-peak-tokens-input")).toBe(40);
    expect(quantityOf(actual, "deepseek-flash-off-peak-tokens-cached-input")).toBe(960);
    expect(quantityOf(actual, "deepseek-flash-off-peak-tokens-output")).toBe(20);
  ...
  it("declares DeepSeek against the peak names on the first minute a peak window opens", async () => {
    ...
    expect(res.status).toBe(200);
    const actual = actualItems(cap.postedItems);
    expect(quantityOf(actual, "deepseek-flash-peak-tokens-input")).toBe(100);
    expect(quantityOf(actual, "deepseek-flash-peak-tokens-cached-input")).toBe(400);
    expect(quantityOf(actual, "deepseek-flash-peak-tokens-output")).toBe(12);
  });
  ...
  it("retries a vendor rate limit rather than throwing the run away", async () => {
    ...
    const actual = cap.postedItems.flat().filter((i) => i.costName.startsWith("zai-glm-5.3"));
  ...
  it("surfaces a persistently saturated vendor as 429, not as a generic failure", async () => {
    ...
    expect(res.body.detail).toMatch(/rate limiting "glm-5\.3"/);
...
describe("POST /internal/platform-complete — direct vendor routing", () => {
  ...
  it("reaches the vendor with its platform key and declares actual costs with the regime + cache split", async () => {
    ...
    expect(quantityOf(actual, "deepseek-flash-peak-tokens-input")).toBe(40);
    expect(quantityOf(actual, "deepseek-flash-peak-tokens-cached-input")).toBe(360);
    expect(quantityOf(actual, "deepseek-flash-peak-tokens-output")).toBe(25);
    expect(actual.some((i) => i.costName === "deepseek-flash-tokens-input")).toBe(false);
