...
describe("buildLlmCostNames — per-vendor dimensions", () => {
  it("DeepSeek: regime segment + cache split", () => {
    expect(
      buildLlmCostNames({ provider: "deepseek", costPrefix: "deepseek-flash", at: at("2026-08-20T02:00:00Z") }),
    ).toEqual({
      input: "deepseek-flash-peak-tokens-input",
      cachedInput: "deepseek-flash-peak-tokens-cached-input",
      output: "deepseek-flash-peak-tokens-output",
    });
    expect(
      buildLlmCostNames({ provider: "deepseek", costPrefix: "deepseek-flash", at: at("2026-08-20T12:00:00Z") }),
    ).toEqual({
      input: "deepseek-flash-off-peak-tokens-input",
      cachedInput: "deepseek-flash-off-peak-tokens-cached-input",
      output: "deepseek-flash-off-peak-tokens-output",
    });
  });

  it("DeepSeek: a former peak hour on a Saturday declares off-peak names", () => {
    expect(
      buildLlmCostNames({ provider: "deepseek", costPrefix: "deepseek-flash", at: at("2026-08-29T02:00:00Z") }),
    ).toEqual({
      input: "deepseek-flash-off-peak-tokens-input",
      cachedInput: "deepseek-flash-off-peak-tokens-cached-input",
      output: "deepseek-flash-off-peak-tokens-output",
  ...
  it("DeepSeek: never declares the superseded regime-free names", () => {
    for (const iso of ["2026-08-20T02:00:00Z", "2026-08-20T12:00:00Z"]) {
      const names = buildLlmCostNames({ provider: "deepseek", costPrefix: "deepseek-flash", at: at(iso) });
      for (const name of [names.input, names.cachedInput!, names.output]) {
        expect(name).toMatch(/-(peak|off-peak)-tokens-/);
      }
      expect(names.input).not.toBe("deepseek-flash-tokens-input");
      expect(names.output).not.toBe("deepseek-flash-tokens-output");
    }
  });
